...
**문자열 검색**: 패턴에 문자 또는 문자열을 지정하면 검색 대상 문자열에서 패턴으로 지정한 문자 또는 문자열을 검색한다.  
**임의의 문자열 검색**: .은 임의의 문자 한 개를 의미한다.  
**반복 검색**: {m,n}은 앞선 패턴이 최소 m번, 최대 n번 반복되는 문자열을 의미하고 +는 앞선 패턴이 최소 한번 이상 반복되는 문자열을 의미하고 ?는 앞선 패턴이 최대 한 번(0번 포함) 이상 반복되는 문자열을 의미한다.  
**OR 검색**: |은 or의 의미를 갖고 [] 내의 문자는 or로 동작한다. 범위를 지정하려면 [] 내에 -를 사용한다.  
**NOT 검색**: [] 내의 ^은 not의 의미를 갖는다.  
**시작 위치로 검색**: [] 밖은 ^은 문자열의 시작을 의미한다.  
