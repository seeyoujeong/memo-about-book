...
프로세스/스레드를 미리 여러 개를 가동시켜 두어서 장애가 발생해도 웹 서버의 서비스 전체가 정지되는 일은 없다.  
...
- DNS가 세션 상태를 파악하지 않기 때문에 다음 접속 시에 동일 서버에 접속해야 하는 경우에도 다른 서버의 주소를 반환한다.
...
- 소스 IP 주소 : 클라이언트 IP 주소를 기반으로 요청을 할당한 웹 서버를 결정한다.
