...
컴퓨터에서 처리하는 데이터는 정해진 메모리 크기 안에 저장해야 한다. 저장할 때는 해당 데이터가 메모리 안에 들어갈지를 판단해야 하기 때문에 상자 크기가 정해져 있는지 여부가 매우 중요하다.  
...
데이터 정리 방법을 '데이터 구조', 처리 순서를 '탐색 알고리즘'이라고 한다.  
...
B 트리 인덱스가 DBMS에서 자주 사용되는 것은 트리 구조 계층이 깊어지지 않도록 디스트 I/O를 최소한으로 제어하기 때문이다.  
