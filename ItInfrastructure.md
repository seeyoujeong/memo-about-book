...
이 구조는 지리적으로 멀리 떨어진 시스템에 자주 이용되거나 각 거점이 완전히 독립된 운영을 하고 있는 경우에 적합하다.
...
일반적으로 캐시 메모리가 커질수록 액세스 속도가 느려진다. 하지만 가능한 CPU 가까운 곳에 많은 캐시를 두고 싶기 때문에 여러 단계로 배치해서 초고속으로 액세스하고 싶은 데이터는 L1 캐시에, 고속으로 액세스하고 싶은 데이터는 L2 캐시에 두는 형태로 만든 것이다.  
...
TCP는 연결형 프로토콜로, 연결이라 불리는 가상 경로(버추얼 서킷)를 생성하는데 3-way handshaking 후 가상 경로가 열린다.  
...
폭주 윈도우 크기는 통신 시작 시 1세그먼트로 설정되고 통신에 문제가 없으면 지수 함수적으로 윈도우 크기를 늘려가다가 어느 정도의 크기까지 증가하면 그 이후는 1세그먼트씩 크기를 늘려간다.  
...
IP 주소를 이용해 대상 서버를 지정할 수 있는데 같은 네트워크 내에 없으면 라우터에게 전송을 부탁한다.  
...
HDD 이중화는 RAID라는 기술을 이용한다.  
RAID는 여러 HDD를 묶어서 그룹으로 만들고 이것을 논리적인 HDD(LU)로 인식하는 기술이다.  
RAID의 장점으로 안정성 확보, 성능 향상, 용량 확장이 있다.  
RAID의 주된 구성 패턴으로 RAID1, RAID5, RAID10이 있다.

> **벌써 시간이 다 됐어?**  
> 하드웨어에는 배터리가 존재한다는 것을 기억해 둬야 한다.  
> 배터리 교환이 수년 단위이기 때문에 쉽게 잊을 수 있다.

### 웹 서버 이중화

#### _프로세스/스레드 이중화_

클라이언트의 http 프로토콜 요청을 받는 것은 웹 서버에서 동작하고 있는 웹 서버 프로그램이다.  
클라이언트 관점에서는 서버 측이 프로세스로 가동되고 있는지, 스레드로 가동되고 있는지를 의식할 필요는 없다.  
프로세스/스레드를 미리 여러 개를 가동시켜 두면 장애가 발생해도 웹 서버의 서비스 전체가 정지되는 일은 없다.  
일반적으로 상용 시스템은 시스템 리소스에 여유가 있으면 가동 프로세스/데몬 수의 최솟값, 최댓값이 같다.  
http 프로세스/스레드가 요청을 받을 수 없는 경우는 서버 측 문제이기 때문에 500번대 에러를 클라이언트에 반환한다.  
웹 서버에 대한 요청은 큐에 쌓이지 않고 바로 에러를 반환한다는 특징이 있다.

#### _서버 이중화_

웹 서버를 이중화하는 방법 중 하나는 DNS를 이용해서 하나의 호스트명에 대해 복수의 IP 주소를 반환하는 것이다.  
이를 DNS 라운드 로빈이라고 하며 DNS는 질의에 대해 순서대로 IP 주소를 반환한다.

DNS 라운드 로빈의 주의 사항은 다음과 같다.

- DNS가 서버 상태를 감시해서 파악하지 않기 때문에 서버가 정지된 경우에도 그 서버의 주소를 반환한다.
- DNS가 세션 상태를 파악하지 않기 때문에 다음 접속 시에 동일 서버에 접속해야 하는 경우에도 다른 서버의 주소를 반환할 수 있다.

더 고도화한 이중화 방식은 부하분산 장치다.  
세션 상태 저장을 실현하는 기능을 부하분산 장치에서는 퍼시스턴스(Persistence) 기능이라고 한다.

주요 퍼시스턴스의 종류는 다음과 같다.

- 소스 IP 주소: 클라이언트 IP 주소를 기반으로 요청을 할당할 웹 서버를 결정한다.
- 쿠키: HTTP 헤더 내에 접속한 웹 서버 정보를 저장한다.
- URL: URL 구조 내에 접속한 웹 서버 정보를 저장한다.

부하분산 장치의 첫 접속 시 서버 할당 알고리즘에도 몇 가지 방식이 존재한다.

- 라운드 로빈(Round Robin): 서버의 IP 주소에 순서대로 요청을 할당한다.
- 최소 연결(Least Connection): 현재 활성 세션 수보다 세션 수가 가장 적은 서버의 IP 주소에 요청을 할당한다.
...
요청 페일오버만 적용하면 동적 콘텐츠일 경우 정보 손실이 발생할 수 있다.  
장애 시에 세션 정보를 유지하기 위해서는 페일오버 이외의 다른 구조가 필요하다.  
할당 알고리즘 선택 시에 고려해야 할 것은 복잡한 알고리즘은 최대한 피하는 것이다.

### AP 서버 이중화

#### _서버 이중화_

요청의 분산 및 세션 정보 이중화 기능을 사용해서 AP 서버를 이중화할 수 있다.  
세션 정보는 접속된 AP 서버를 기본으로 하고 보조 세션을 복사해 둔다.  
클라이언트가 재접속할 때는 웹 서버에 구현된 리디랙션용 플러그인이 기본 서버를 판별해서 해당하는 AP 서버에 요청을 리디랙션한다.  
장애 시에는 쿠키 정보를 가지고 보조 세션 정보에 접속해서 세션을 계속 유지시킨다.

#### _DB 연결 이중화_

AP 서버는 DB 서버에 접속 시에 사용할 연결을 사전에 여러 개 생성해서 데이터베이스 처리를 병렬로 실행할 수 있다.(연결 풀링)  
감시에 두 번 실패하면 연결을 일단 끊은 후 재접속하고 연결이 모두 사용 중인 경우에는 일정 시간 대기하면서 경계 값에 도달하면 에러를 반환한다.  
중간에 방화벽이 있다면 오랫동안 사용하지 않은 세션을 자동으로 제거하는 경우가 있기 때문에 방화벽 유무를 확인해 두어야 한다.
